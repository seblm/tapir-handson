...
class PodcastCSV(resourceName: String) extends PodcastRepository {
  ...
  val podcasts: Seq[Podcast] = Source
  ...
  private def toPodcast(columns: List[String]): Option[Podcast] = {
    Some(
      Podcast(
        ...
        techRssURL = Try(columns(11)).flatMap(urlAsString => Try(new URL(urlAsString))).toOption,
        techRssURLRedirect = Try(columns(12)).flatMap(urlAsString => Try(new URL(urlAsString))).toOption,
        techImage = Try(columns(13)).flatMap(urlAsString => Try(new URL(urlAsString))).toOption,
        techLinkURL = Try(columns(14)).flatMap(urlAsString => Try(new URL(urlAsString))).toOption,
